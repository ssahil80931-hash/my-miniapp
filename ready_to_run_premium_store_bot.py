...
#
# IMPORTANT:
# Telegram bot tokens are secrets. Keep BOT_TOKEN in Railway Variables.
...
# Shown before the saved start videos.
OFFER_MESSAGE = "🔥 NEW OFFER\n\nTap /start to open the store and view the latest categories."
...
BUTTON_STYLES = {
    "cat1": "primary", "cat2": "success", "cat3": "danger", "cat4": "secondary",
    "cat5": "primary", "cat6": "success", "cat7": "danger", "cat8": "secondary",
}
...
def category_keyboard():
    ...
    if support:
        bottom.append(styled_button("💬 SUPPORT", url=support))
    rows.append(bottom)
...
async def send_store(update: Update):
    ...
    await message.reply_text(OFFER_MESSAGE)
    ...
    if welcome:
        await message.reply_text(welcome, reply_markup=category_keyboard())
    else:
        await message.reply_text("👇 CHOOSE A CATEGORY", reply_markup=category_keyboard())
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # IMPORTANT: /start never resets setup. Once setup_complete is true,
    # admin gets the same store flow as every other user.
...
async def category_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    support = support_url()
    if support:
        rows.append([styled_button("💬 SUPPORT", url=support)])
...
async def home_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.answer()
    welcome = data["welcome_message"].strip()
    await query.message.reply_text(welcome or "👇 CHOOSE A CATEGORY", reply_markup=category_keyboard())


async def offer_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    await query.message.reply_text(OFFER_MESSAGE, reply_markup=InlineKeyboardMarkup([
...
def admin_panel_keyboard():
    return InlineKeyboardMarkup([
        [styled_button("🎬 SET START VIDEOS", "admin:startvideos", "primary")],
        [styled_button("📝 SET WELCOME MESSAGE", "admin:welcome", "success")],
        [styled_button("📂 CATEGORY SETUP", "admin:categories", "danger")],
        [styled_button("🚀 COMPLETE SETUP", "admin:complete", "secondary")],
        [styled_button("📢 BROADCAST", "admin:broadcast", "primary")],
    ])
...
async def admin_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("👑 ADMIN PANEL\n\nChoose an option:", reply_markup=admin_panel_keyboard())
...
def category_admin_keyboard():
    ...
    for i in range(0, len(keys), 2):
        ...
        for key in keys[i:i + 2]:
            cat = data["categories"][key]
            row.append(styled_button(f"{key.upper()} • {cat['name']}", f"admincat:{key}", BUTTON_STYLES[key]))
        rows.append(row)
...
def category_edit_keyboard(key):
    return InlineKeyboardMarkup([
        ...
        [styled_button("🎬 ADD VIDEOS", f"addcatvideos:{key}", "secondary")],
...
async def admin_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "panel":
        ...
        await query.message.reply_text("🎬 Send 1–6 start videos, one by one. Send /done when finished.")
    elif action == "welcome":
        context.user_data["mode"] = "welcome"
        await query.message.reply_text("📝 Send the welcome message now. Send /done after it is saved.")
    elif action == "categories":
        await query.message.reply_text("📂 Select the category you want to edit/setup:", reply_markup=category_admin_keyboard())
    elif action == "complete":
        data["setup_complete"] = True
        save_data()
        await query.message.reply_text("✅ Setup completed. From now on /start opens the store normally.\n\nUse /admin for admin settings.")
    elif action == "broadcast":
        context.user_data["mode"] = "broadcast"
        await query.message.reply_text("📢 Send the broadcast text/photo/video now. Send /cancel to cancel.")
...
async def edit_category_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "editname":
        ...
        await query.message.reply_text(f"✏️ Send the new name for {key.upper()}.")
    elif action == "editdesc":
        context.user_data["mode"] = "category_description"
        await query.message.reply_text(f"📝 Send the new description for {key.upper()}.")
    elif action == "editprice":
        context.user_data["mode"] = "category_price"
        await query.message.reply_text(f"💰 Send the new price (numbers only) for {key.upper()}.")
    elif action == "addcatvideos":
        context.user_data["mode"] = "category_videos"
        await query.message.reply_text(f"🎬 Send up to 5 videos for {key.upper()}, one by one. Send /done when finished.")
...
async def admin_message_router(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if mode == "welcome":
        ...
        await msg.reply_text("✅ Welcome message saved. Use /done or /admin to continue.")
        return
    ...
    if mode == "start_videos":
        ...
        save_data()
        await msg.reply_text(f"✅ Start video saved: {len(data['start_videos'])}/6. Send another or /done.")
        return
    ...
    if mode == "category_videos":
        ...
        save_data()
        await msg.reply_text(f"✅ Video saved to {key.upper()}: {len(data['videos'][key])}/5. Send another or /done.")
        return
    ...
    if mode == "category_name":
        ...
        save_data()
        context.user_data.clear()
        await msg.reply_text(f"✅ {key.upper()} name updated.\nNew name: {data['categories'][key]['name']}", reply_markup=category_admin_keyboard())
        return
    ...
    if mode == "category_description":
        ...
        save_data()
        context.user_data.clear()
        await msg.reply_text(f"✅ {key.upper()} description updated.", reply_markup=category_admin_keyboard())
        return
    ...
    if mode == "category_price":
        ...
        save_data()
        context.user_data.clear()
        await msg.reply_text(f"✅ {key.upper()} price updated to ₹{price}.", reply_markup=category_admin_keyboard())
        return
...
async def done(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if mode in {"start_videos", "category_videos"}:
        ...
        context.user_data.clear()
        await update.message.reply_text(f"✅ Video setup finished. Saved: {saved}")
    elif mode == "welcome":
        context.user_data.clear()
...
def main():
    ...
    # All admin setup content goes through one router, preventing the
    # previous video/broadcast handler conflict.
